...
$date = $_POST['date'];
$time = $_POST['time'];
$save1=mysql_query("INSERT INTO schedule (barangay,date,time) VALUES ('$barangay','$date','$time')");

echo "<script type=\"text/javascript\">window.alert('Vaccination Schedule has been added');window.location.href = 'schedule.php';</script>"; 
}    
?>
		  <form action="#" method="POST">
		  <h3 style="margin:0px">Add Schedule</h2>
			Barangay:<br>
			<select name="barangay" class="email" required>
								<option></option>
...
$result = mysql_query("SELECT * FROM schedule  ORDER BY date DESC");
while($row = mysql_fetch_array($result))
{
	$time = $row['time'];
	$time =  date('h:i A ', strtotime($time));
			echo '<tr>';
			echo '<td valign="top">&nbsp;'.$row['barangay'].'</td>';
